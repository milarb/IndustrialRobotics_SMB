...
for i=1:steps
    x(1,i) = -0.3; % Points in x
    x(2,i) = (1-s(i))*-0.3 + s(i)*0.3; % Points in y
    x(3,i) = 0; % Points in z   
end
 
T = transl(-0.3,-0.3,0);          % Create transformation of first point and angle
q0 = [ 0.4363   -0.1920    0.7540   -2.1328   -1.5708   -2.1328];                                                            % Initial guess for joint angles
...
for i = 1:steps-1
    ...
    T = ur3.model.fkine(qMatrix(i,:)).T;                                           % Get forward transformation at current joint state
